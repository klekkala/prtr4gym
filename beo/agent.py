...
class Agent():
    ...
    def reset(self):
        ...
        self.curr_angle = 0
        if self.pano_mode:
            self.curr_view = self.dh.panorama_split(self.curr_angle, self.agent_pos_curr, self.view_res)
        else:
    ...
    def update_agent(self, new_pos, curr_pos, new_angle):
        ...
        if self.pano_mode:
            self.curr_view = self.dh.panorama_split(self.curr_angle, curr_pos, self.view_res)
        else:
            self.curr_view = self.dh.get_image_orientation(self.agent_pos_curr, self.curr_camera)
