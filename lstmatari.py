...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log", type=str, default="/lab/kiran/logs/rllib/lstmAtari", help="config file for resources"
    )
    parser.add_argument(
        "--env_name", type=str, default="SpaceInvadersNoFrameskip-v4", help="AirRaidNoFrameskip-v4"
    )
    parser.add_argument(
    ...
    class MultiTaskEnv(gym.Env): 
        def __init__(self, env_config):    
            self.env = gym.make(args.env_name, full_action_space=True)
    ...
    str_logger =args.env_name + "_" + str(args.stop_timesteps) + "_lr" + str(args.lr) + "_lam" + str(args.lambda_) + "_kl" + str(args.kl_coeff) + "_cli" + str(args.clip_param) + "_ent" + str(args.entropy_coeff) + "_gam" + str(args.gamma) + "_buf" + str(args.buffer_size) + "_bat" + str(args.batch_size) + "_num" + str(args.num_epoch)
    if args.model=='fcnet':
        config = {
        "env" : MultiTaskEnv,
