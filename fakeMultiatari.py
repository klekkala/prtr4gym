...
if __name__ == "__main__":
    ...
    for env in envs:
        ...
        if args.tune == False:
            ...
            if env!=envs[0]:
                my_restored_policy = Policy.from_checkpoint("./multiatari/atari_checkpoint")
                policy=algo.get_policy()
                logits_shape = my_restored_policy.get_weights()['_logits._model.1.weight'].shape
                bias_shape = (my_restored_policy.get_weights()['_logits._model.1.bias'].shape[0],1)
                logits_layer = torch.nn.Parameter(torch.nn.init.xavier_normal_(torch.Tensor(*logits_shape)))
                bias_layer = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(*bias_shape))).squeeze()
                logits_layer=logits_layer.detach().numpy()
                bias_layer = bias_layer.detach().numpy()
                temp = my_restored_policy.get_weights()
                temp['_logits._model.1.weight'] = logits_layer
                temp['_logits._model.1.bias'] = bias_layer
                policy.set_weights(temp)
