...
class SOMContThreeChan(BaseDataset):
    def __init__(self, root_dir, sample_next, transform=None, value=False, episode=True, goal=True):
    ...
    def __getitem__(self, item):
        ...
        img = [np.moveaxis(self.obs_nps[file_ind][im_ind].astype(np.float32), -1, 0), np.moveaxis(self.obs_nps[file_ind][im_ind + deltat].astype(np.float32), -1, 0)]
        aux = [self.aux_nps[file_ind][im_ind].astype(np.float32), self.aux_nps[file_ind][im_ind + deltat].astype(np.float32)]
        #return np.stack(img, axis=0), np.stack(value, axis=0), np.stack(episode, axis=0)
