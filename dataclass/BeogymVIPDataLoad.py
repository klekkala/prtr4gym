...
class BeogymVIPDataLoad(BaseDataset):
    ...
    def __getitem__(self, item):
        ...
        mid_img = np.moveaxis(self.obs_nps[file_ind][mid_ind].astype(np.float32), -1, 0)
        midplus_img = np.moveaxis(self.obs_nps[file_ind][midplus].astype(np.float32), -1, 0)

        start_aux = self.aux_nps[file_ind][start_ind].astype(np.float32)
        last_aux = self.aux_nps[file_ind][end_ind].astype(np.float32)
        mid_aux = self.aux_nps[file_ind][mid_ind].astype(np.float32)
        midplus_aux = self.aux_nps[file_ind][midplus].astype(np.float32)

        
        return np.stack([start_img, mid_img, midplus_img, last_img], axis=0), np.stack([start_aux, mid_aux, midplus_aux, last_aux], axis=0)
