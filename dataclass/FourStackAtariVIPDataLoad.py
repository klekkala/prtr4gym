...
class FourStackAtariVIPDataLoad(BaseDataset):
    ...
    def __getitem__(self, item):
        ...
        start_img = self.obs_nps[file_ind][start_ind].astype(np.float32)
        last_img = self.obs_nps[file_ind][end_ind].astype(np.float32)

        mid_img = self.obs_nps[file_ind][mid_int].astype(np.float32)
        midplus_img = self.obs_nps[file_ind][midplus].astype(np.float32)
